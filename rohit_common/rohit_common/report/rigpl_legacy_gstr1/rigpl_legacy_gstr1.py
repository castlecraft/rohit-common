...
from india_compliance.gst_india.report.gst_advance_detail.gst_advance_detail import get_gst_accounts
...
class Gstr1Report(object):
	...
	def run(self):
		...
		# Normalize get_gst_accounts return into expected structure
		raw_gst_accounts = get_gst_accounts(self.filters)
		...
		# If get_gst_accounts returns a list of account names (your env):
		if isinstance(raw_gst_accounts, (list, tuple)):
			for ac in raw_gst_accounts:
				if not ac:
					continue
				name = ac.lower()
				if "cgst" in name:
					gst_accounts.cgst_account.append(ac)
				elif "sgst" in name:
					gst_accounts.sgst_account.append(ac)
				elif "igst" in name:
					gst_accounts.igst_account.append(ac)
				elif "cess" in name:
					gst_accounts.cess_account.append(ac)

		# If it returned a mapping-like object (other environments)
		elif hasattr(raw_gst_accounts, "get"):
			for key in ("cgst_account", "sgst_account", "igst_account", "cess_account"):
				gst_accounts[key] = raw_gst_accounts.get(key, gst_accounts.get(key))

		# If it returned an object with attrs
		elif hasattr(raw_gst_accounts, "__dict__"):
			gst_accounts.cgst_account = getattr(raw_gst_accounts, "cgst_account", gst_accounts.cgst_account)
			gst_accounts.sgst_account = getattr(raw_gst_accounts, "sgst_account", gst_accounts.sgst_account)
			gst_accounts.igst_account = getattr(raw_gst_accounts, "igst_account", gst_accounts.igst_account)
			gst_accounts.cess_account = getattr(raw_gst_accounts, "cess_account", gst_accounts.cess_account)

		# assign normalized structure back to self
	...
	def get_items_based_on_tax_rate(self):
		...
		for parent, account, item_wise_tax_detail, tax_amount in self.tax_details:
			# item_wise_tax_detail might be a JSON string in DB
			if item_wise_tax_detail and isinstance(item_wise_tax_detail, str):
				try:
					item_wise_parsed = json.loads(item_wise_tax_detail)
				except Exception:
					item_wise_parsed = None
			else:
				item_wise_parsed = item_wise_tax_detail
			...
			# If no item-wise detail, continue
			if not item_wise_parsed:
				continue

			try:
				# Determine whether this account is CGST/SGST or IGST
				cgst_or_sgst = False
				...
				# item_wise_parsed expected format can be:
				# { "ITEM_CODE": [rate, amount, ...] }  (legacy)
				# OR { "ITEM_CODE": {"tax_rate": 18.0, ...} }  (india_compliance)
				for item_code, tax_amounts in iteritems(item_wise_parsed):
					# Determine tax_rate from either list/tuple or dict
					tax_rate = None
					if isinstance(tax_amounts, (list, tuple)) and len(tax_amounts) > 0:
						# legacy format: first element is rate
						tax_rate = tax_amounts[0]
					elif isinstance(tax_amounts, dict):
						# new format: look for 'tax_rate' or 'rate'
						tax_rate = tax_amounts.get("tax_rate") or tax_amounts.get("rate") or 0
					else:
						# fallback: try to coerce numeric if possible
						try:
							tax_rate = float(tax_amounts)
						except Exception:
							tax_rate = 0
					...
					if cgst_or_sgst:
						# legacy representation multiplied CGST/SGST halves; to get total rate, multiply by 2
						tax_rate *= 2
