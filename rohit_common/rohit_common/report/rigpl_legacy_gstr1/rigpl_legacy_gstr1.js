...
frappe.query_reports["RIGPL Legacy GSTR1"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
	...
	onload: function (report) {
		...
		report.page.add_inner_button(__("Download as Json"), function () {
			...
			frappe.call({
				method: 'rohit_common.report.rigpl_legacy_gstr1.get_json',
				args: {
					data: report.data,
				...
				callback: function(r) {
					if (r.message) {
						const args = {
							cmd: 'rohit_common.report.rigpl_legacy_gstr1.download_json_file',
							data: r.message.data,
